...
export class Camera {
  ...
  public async playAnim(
    ...
    await global.LoadAnimDict(animDict);
    global.PlayCamAnim(
      this.handle,
      animName,
      animDict,
    ...
    global.RemoveAnimDict(animDict);
  }

  public pointAt(target: Entity | PedBone | Vector3, offset: Vector3 = new Vector3(0, 0, 0)): void {
    if (target instanceof Entity) {
      global.PointCamAtEntity(this.handle, target.Handle, offset.x, offset.y, offset.z, true);
    } else if (target instanceof PedBone) {
      global.PointCamAtPedBone(
        this.handle,
        target.Owner.Handle,
        target.Index,
        offset.x,
      ...
      global.PointCamAtCoord(this.handle, target.x, target.y, target.z);
    }
  }

  public stopPointing(): void {
    global.StopCamPointing(this.handle);
  }
  ...
  public get IsInterpolating(): boolean {
    return global.IsCamInterpolating(this.handle);
  }

  public attachTo(object: Entity | PedBone, offset: Vector3): void {
    if (object instanceof Entity) {
      global.AttachCamToEntity(this.handle, object.Handle, offset.x, offset.y, offset.z, true);
    } else if (object instanceof PedBone) {
      global.AttachCamToPedBone(
        this.handle,
        object.Owner.Handle,
        object.Index,
        offset.x,
  ...
  public detach(): void {
    global.DetachCam(this.handle);
  }

  public delete(): void {
    global.DestroyCam(this.handle, false);
  }

  public exists(): boolean {
    return global.DoesCamExist(this.handle);
  }
}
