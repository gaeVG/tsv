...
const drawText = (text: string, position: number[], options: any): void => {
  ...
  global.SetTextEntry('CELL_EMAIL_BCON');
  for (let i = 0; i < text.length; i += 99) {
    const subStringText = text.substr(i, Math.min(99, text.length - i));
    global.AddTextComponentSubstringPlayerName(subStringText);
  }

  global.SetTextFont(font);
  global.SetTextScale(scale, scale);
  global.SetTextColour(color[0], color[1], color[2], color[3]);
  ...
  if (outline) {
    global.SetTextOutline();
  }

  switch (align) {
    case 1: {
      global.SetTextCentre(true);
      break;
    }

    case 2: {
      global.SetTextRightJustify(true);
      global.SetTextWrap(0.0, position[0] || 0);
      break;
    }
  }

  global.DrawText(position[0] || 0, position[1] || 0);
};
...
export class Timerbar {
  ...
  constructor(title: string, useProgressBar = false) {
    ...
    const safeZone = global.GetSafeZoneSize();
  ...
  public set TextColor(value: number | number[]) {
    if (Array.isArray(value)) {
      ...
      const result = global.GetHudColour(value);
