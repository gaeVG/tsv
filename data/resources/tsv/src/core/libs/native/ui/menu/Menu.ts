...
export class Menu {
  ...
  private _render(): void {
    ...
    global.SetScriptGfxAlign(this._alignment, 84);

    const menuWidth = (431 + this._widthOffset) / Menu.screenWidth;

    if (this.Settings.scaleWithSafezone) {
      global.SetScriptGfxAlignParams(0, 0, menuWidth, 0);
      const pos = global.GetScriptGfxPosition(0, 0);
      this._drawOffset.X = pos[0];
      this._drawOffset.Y = pos[1];
    } else {
      const sSize = (1 - global.GetSafeZoneSize()) / 2;
      if (this._alignment === MenuAlignment.Right) {
        global.SetScriptGfxAlignParams(sSize, -sSize, menuWidth, 0);
        const pos = global.GetScriptGfxPosition(0, 0);
        this._drawOffset.X = pos[0];
        this._drawOffset.Y = pos[1];
      } else {
