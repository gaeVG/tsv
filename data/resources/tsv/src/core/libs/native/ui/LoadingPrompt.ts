...
export abstract class LoadingPrompt {
  ...
  public static show(
    ...
    if (loadingText === '') {
      global.BeginTextCommandBusyString('');
    } else {
      global.BeginTextCommandBusyString('STRING');
      global.AddTextComponentSubstringPlayerName(loadingText);
    }

    global.EndTextCommandBusyString(Number(spinnerType));
  }

  public static hide(): void {
    if (this.IsActive) {
      global.RemoveLoadingPrompt();
    }
  }

  public static get IsActive(): boolean {
    return global.IsLoadingPromptBeingDisplayed();
  }
}
