...
export class Text implements IDrawable {
  public static draw(
    ...
    global.SetTextFont(Number(font));
    global.SetTextScale(1.0, scale);
    global.SetTextColour(color.r, color.g, color.b, color.a);

    if (dropShadow) {
      global.SetTextDropshadow(2, 0, 0, 0, 0);
    }
    ...
    switch (alignment) {
      ...
      case Alignment.Right:
        ...
        if (!wordWrap) {
          global.SetTextWrap(0.0, x);
        }
        break;
    }

    if (wordWrap) {
      global.SetTextWrap(x, (pos.X + wordWrap.width) / resolution.width);
    }

    global.SetTextEntry('STRING');
  ...
  public static addLongString(str: string): void {
    ...
    for (let i = 0; i < str.length; i += strLen) {
      ...
      global.AddTextComponentSubstringPlayerName(substr);
    }
  }
