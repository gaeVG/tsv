...
export class InstructionalButtons {
  ...
  private pushButton(button: IButton, index: number) {
    global.BeginScaleformMovieMethod(this.scaleform.Handle, 'SET_DATA_SLOT');
    global.PushScaleformMovieFunctionParameterInt(index);
    ...
    for (let i = button.controls.length - 1; i >= 0; i--) {
      global.PushScaleformMovieMethodParameterButtonName(
        global.GetControlInstructionalButton(2, button.controls[i], true),
      );
    }
    global.PushScaleformMovieMethodParameterString(button.label);
