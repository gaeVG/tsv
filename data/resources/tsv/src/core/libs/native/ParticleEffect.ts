...
export abstract class ParticleEffect {
  ...
  public get IsActive(): boolean {
    return this.Handle !== -1 && global.DoesParticleFxLoopedExist(this.Handle);
  }
  ...
  public stop(): void {
    if (this.IsActive) {
      global.RemoveParticleFx(this.Handle, false);
    }
    this.handle = -1;
  }
  ...
  public set Rotation(rotation: Vector3) {
    ...
    if (this.IsActive) {
      ...
      global.SetParticleFxLoopedOffsets(
        this.Handle,
        off.x,
        off.y,
        off.z,
  ...
  public set Range(range: number) {
    this.range = range;
    global.SetParticleFxLoopedRange(this.Handle, range);
  }
  ...
  public setParameter(parameterName: string, value: number): void {
    if (this.IsActive) {
      global.SetParticleFxLoopedEvolution(this.Handle, parameterName, value, false);
    }
  }
