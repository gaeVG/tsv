...
import { PlayerComponentEnum, PlayerComponentType } from "../../../../core/declares/inventory";

function PlayerComponents () {
  const [playerComponents, setPlayerComponents] = useState([] as Array<{ name: string, components: PlayerComponentType[]}>);
    ...
    playerComponents.length > 0 && (
      <div id="playerComponents"> 
        {playerComponents.map((playerComponent) => (
          <article className={`playerComponent-type playerComponent-type-${playerComponent.name}`} key={playerComponent.name}>
